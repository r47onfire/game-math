...
test("supports paused owners without depending on GameObj", () => {
    const handler = mock();
    const event = new SingleEvent<void, { paused: boolean }>(owner => owner.paused);
    const controller = event.add(handler);
    controller.owner = { paused: true };
    event.fire();
...
test("cancel symbol works to cancel", () => {
    const handler = mock(() => STOP_EVENT) as () => typeof STOP_EVENT;
    const event = new SingleEvent<void>();
    event.add(handler);
    event.fire();
    event.fire();
